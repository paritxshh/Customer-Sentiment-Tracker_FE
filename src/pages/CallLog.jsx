...
export default function CallLog() {
  ...
  useEffect(() => {
    ...
    const hasTranscript =
      chatDetail.transcript &&
      Array.isArray(chatDetail.transcript) &&
      chatDetail.transcript.length > 0;
    const hasSummary =
      chatDetail.callSummary && String(chatDetail.callSummary).trim();
    if (
      !hasTranscript ||
      hasSummary ||
      autoSummaryRequestedRef.current.has(chatDetail.id)
    )
      return;
    ...
    generateChatSummary(chatDetail.id)
      .then((r) => {
        if (r.ok && r.callSummary) {
          setChatDetail((prev) =>
            prev ? { ...prev, callSummary: r.callSummary } : null,
          );
        }
      })
      .catch(() => {})
              ...
              <dl className="space-y-3">
                {Object.entries(details).map(([key, value]) => (
                  <div key={key}>
                    ...
                    <dd className="text-sm text-gray-200 mt-0.5 font-mono break-all">
                      {value}
                    </dd>
                  </div>
                ))}
